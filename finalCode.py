...
def getAttributess(url):
    ...
    d={'Protocol':pd.Series(protocol),'Domain':pd.Series(domain),'Path':pd.Series(path),'Having_@_symbol':pd.Series(having_at_symbol),'Having_IP':pd.Series(having_ip),
   'Prefix_suffix_separation':pd.Series(prefix_suffix_separation),'Redirection_//_symbol':pd.Series(redirection_symbol),'Sub_domains':pd.Series(sub_domains),
...
url = pd.read_csv("out.csv")
#print(url['Unnamed: 0'])
urls = url.drop('in',axis=1)
#print(urls['0'],urls.columns)
urls = urls.drop('0',axis=1)
#print(urls)
...
data_train, data_test, labels_train, labels_test = train_test_split(urls_without_labels, labels, test_size=0.10, random_state=42)
...
result=getAttributess(u)

r=random_forest_classifier.predict(result)
for i in r:
    if r==1:
        print("PHISHING")
    else:
